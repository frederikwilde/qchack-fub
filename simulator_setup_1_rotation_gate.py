...
pi_pulse_90_degree = pulse_lib.gaussian(duration=drive_samples,
                                 amp=pi_amp_01*2, # change the amplitude to change the gate
...
# +
# Ground state schedule
X90_schedule = pulse.Schedule(name="X90 schedule")
X90_schedule |= pulse.Play(pi_pulse_90_degree, drive_chan) 
X90_schedule |= measure << X90_schedule.duration

X90_schedule.draw(label=False)

# -

NUM_SHOTS=1000
X90_program = assemble([X90_schedule],
                        backend=backend,
                        meas_level=1,
                        meas_return='single',
                        shots=NUM_SHOTS,
...
X90_job = backend.run(X90_program)

X90_data = get_job_data(X90_job, average=False)
print(X90_data)
np.cos( np.pi/4)


def X90_plot(x_min, x_max, y_min, y_max):
    """Helper function for plotting IQ plane for |0>, |1>. Limits of plot given
    as arguments."""
    #zero data plotted in blue
    plt.scatter(np.real(X90_data), np.imag(X90_data))

    # Plot a large dot for the average result of the zero and one states.
    mean_X90 = np.mean(X90_data) # takes mean of both real and imaginary parts
    plt.scatter(np.real(mean_X90), np.imag(mean_X90), 
                s=200, cmap='viridis', c='black',alpha=1.0)
    print(np.real(mean_X90))
...
X90_plot(x_min, x_max, y_min, y_max)
